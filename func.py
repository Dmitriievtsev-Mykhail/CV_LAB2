...
def distanceTransform(image: np.ndarray, start_point: tuple) -> np.ndarray:


    object_mask = getRegion(image, start_point)
    ...
    dist_map_normalized = (dist_map - np.min(dist_map)) / (np.max(dist_map) - np.min(dist_map)) * 255
    dist_map_normalized = np.clip(dist_map_normalized, 0, 255).astype(np.uint8)
